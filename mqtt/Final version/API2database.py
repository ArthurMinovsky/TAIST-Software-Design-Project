...
app_db1 = mongoClient.car_in # Database for in
app_db2 = mongoClient.car_out#Database for out
...
@app.route("/api/inject_logging_in", methods=['POST'])
def inject_logging():
    #if os.environ["PHASE"] == "DEVELOPMENT":
    data = request.get_json()
    data['timestamp'] = datetime.now()
    logging_col = app_db1.logging # Collection
    logging_col.insert_one(data)
    print(data)
    return jsonify({"status": "OK"})
    #return jsonify({"status": "ERROR"})

@app.route("/api/inject_logging_out", methods=['POST'])
def inject_logging():
    #if os.environ["PHASE"] == "DEVELOPMENT":
    data = request.get_json()
    data['timestamp'] = datetime.now()
    logging_col = app_db2.logging # Collection
    logging_col.insert_one(data)
    print(data)
    return jsonify({"status": "OK"})
    #return jsonify({"status": "ERROR"})
